...
void Server::handleTopic(int client_fd, const std::vector<std::string> &tokens)
{
    ...
    if(tokensSize == 2)
    {
        if (channels[channel_name]->isTopicSet())
            throw std::runtime_error(":ircserv 332 " + clientsFds[client_fd]->getNick() + " " + channel_name + " :" + channels[channel_name]->getTopic());
        throw std::runtime_error(":ircserv 331 " + clientsFds[client_fd]->getNick() + " " + channel_name + " :No topic is set");
    }
    if (channels[channel_name]->isTopicProtected() && !channels[channel_name]->getOperators().count(clientsFds[client_fd]))
    ...
    channels[channel_name]->brodcastMsg(":" + clientsFds[client_fd]->getNick() + " TOPIC " + channel_name + " :" + topic, NULL);
}